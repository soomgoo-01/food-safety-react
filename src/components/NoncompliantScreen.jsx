          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {ftList.map(([type, cnt], i) => (
              <div key={type} onClick={() => setTypeModal(type)}
                style={{ display: "flex", gap: 12, alignItems: "center", cursor: "pointer", padding: "8px 12px", borderRadius: 8, background: i === 0 ? "#fef2f2" : "#f9fafb", border: i === 0 ? "1px solid #fca5a5" : "1px solid #f3f4f6" }}>
                ...
                <div style={{ flex: 1, background: "#e5e7eb", borderRadius: 4, height: 20 }}>
                  <div style={{ width: `${(cnt / maxFT) * 100}%`, background: i === 0 ? "#dc2626" : i === 1 ? "#ea580c" : "#2563eb", height: 20, borderRadius: 4, display: "flex", alignItems: "center", paddingLeft: 8 }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {Object.keys(prevYear).map(type => {
              ...
              return (
                <div key={type} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderRadius: 8, background: inc ? "#fef2f2" : "#f9fafb", border: inc ? "1px solid #fca5a5" : "1px solid #f3f4f6" }}>
                    ...
                    {allItems.map(item => {
                      ...
                      return (
                        <td key={item} onClick={() => cnt > 0 && setHeatModal({ type, item })}
                          style={{ padding: "8px 10px", textAlign: "center", background: cnt > 0 ? `rgba(220,38,38,${0.15 + intensity * 0.7})` : "#f9fafb", cursor: cnt > 0 ? "pointer" : "default", borderRadius: 4, color: cnt > 0 ? "#fff" : "#d1d5db", fontWeight: 700 }}>
                          {cnt || ""}
                        </td>
                      );
                    })}
        ...
        <Modal title={`[${typeModal}] 시험항목별 부적합 건수 (건수 순)`} onClose={() => setTypeModal(null)}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {getItemsForType(typeModal).map(([item, cnt], i) => {
              ...
              return (
                <div key={item} style={{ display: "flex", gap: 12, alignItems: "center", padding: "8px 10px", background: i === 0 ? "#fef2f2" : "#f9fafb", borderRadius: 8 }}>
                  ...
                  <div style={{ flex: 1, background: "#e5e7eb", borderRadius: 4, height: 18 }}>
                    <div style={{ width: `${(cnt / max) * 100}%`, background: i === 0 ? "#dc2626" : "#2563eb", height: 18, borderRadius: 4, display: "flex", alignItems: "center", paddingLeft: 8 }}>
                      <span style={{ fontSize: 11, color: "#fff", fontWeight: 600 }}>{cnt}건</span>
                    </div>
                  </div>
                </div>
